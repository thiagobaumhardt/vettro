...
<template>
  ...
  <div class="card">
    ...
    <div v-for="c in hist" :key="c.id" class="cobr-card" :class="{ pago: c.status === 'pago' }">
      <div class="cobr-top">
        <div>
          <h4 style="font-size: 0.95rem; color: #2b2b22">
            {{ new Date(c.criado_em).toLocaleString("pt-BR") }}
            <span class="badge" :class="c.status === 'pago' ? 'b-pago' : 'b-pendente'">{{ c.status === "pago" ? "✔ Pago" : "⏳ Pendente" }}</span>
          </h4>
          <div v-if="c.tutor_nome" style="font-size: 0.8rem; color: #6e6c5c; margin-top: 3px">Tutor: {{ c.tutor_nome }}</div>
        </div>
      ...
      <div v-if="c.obs" style="font-size: 0.83rem; color: #6e6c5c; margin-top: 4px">📝 {{ c.obs }}</div>
